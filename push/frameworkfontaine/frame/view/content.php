...
class ContentViewer extends BuildHTML {
	...
	//format the content - output parallel view for text, text2 and image. 
	...
	//making a statement where if image is empty, default to text2 table
	else {
	$img = $content_txt2;
	}
	$txt_output = $txt_tab_start.$txt.$content_end;
	$img_output = $img_tab_start.$img.$content_end;
	//I liked the text better on the left than the right
	self::$viewer_content = $txt_output.$img_output;
	}
