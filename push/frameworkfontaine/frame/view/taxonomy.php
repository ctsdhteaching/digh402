...
class TaxonomyViewer extends BuildHTML {
	...
	$headers = array("Title","Text Preview","Image","Text Link", "Image link");
	...
	$link_title_attributes = array("href"=>'?node=content/text&id='.$item_id,"class"=>TAXA_LINK);
	...
	if (!empty($item_text)) {
	$item_text_attributes = array("title"=>"");
	...
	$img_attributes = array("src"=>"media/images/".$item_img,"title"=>"");
	...
	//table cells for links to full text and image, no tips w/error hand
	$item_link_attributes = array("title"=>"");
	...
	self::$taxa_content .= $item_row_start.$item_title_cell.$item_text_cell.$item_img_cell.$text_link_cell.$img_link_cell.$row_end;
	}
	return self::$taxa_content;
	}
	
	//format table headings, no tips
