...
class View extends BuildHTML {
	...
	//continue to use the $li_start & $li_end
	echo $li_start.'<li id="'.$parent_id.'"><a href="?node='.$parent_link.'" title="'.$parent_desc.'">'.$parent_name.'</a>'.$li_end;
	if (is_array($val)) {
	echo $ul_start;
	...
	//continue to use the $li_start & $li_end
	echo $li_start.'<li id="'.$child_id.'"><a href="?node='.$child_link.'" title="'.$child_desc.'">'.$child_name.'</a>'.$li_end;
	}
	}
	echo $ul_end.$li_end;
	}
	...
	//Making the logo a clickable insite link		
		echo '<a href="?node=content/text&id=1', 'grid_3">'.$logo_start.'</a>';
		echo $header_end;
	}
